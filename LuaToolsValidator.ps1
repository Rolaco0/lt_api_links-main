...
foreach ($url in $validatorUrls) {
    try {
        Write-Host "[*] Downloading latest Rolaco Tools Validator..." -ForegroundColor Cyan
...
if (-not $downloaded) {
    Write-Host "[-] Could not download RolacoTools Validator from any mirror." -ForegroundColor Red
...
Write-Host "[*] Replacing old RolacoTools Validator..." -ForegroundColor Cyan
Get-Process -Name "RolacoToolsValidator" -ErrorAction SilentlyContinue | ForEach-Object {
    try {
...
Write-Host "[+] RolacoTools Validator is updated." -ForegroundColor Green
...
function Start-LuaToolsValidator {
    ...
    try {
        ...
        if ($proc.HasExited) {
            Write-Host "[-] RolacoTools Validator closed immediately. Exit code: $($proc.ExitCode)" -ForegroundColor Red
        ...
        Write-Host "[-] Failed to start RolacoTools Validator: $($_.Exception.Message)" -ForegroundColor Red
...
if ($LuaToolsInstallOnly) {
    Write-Host "[+] Starting RolacoTools Validator..." -ForegroundColor Green
    Start-LuaToolsValidator -FilePath $exePath
}
else {
    Write-Host "[+] Starting RolacoTools Validator for AppID $AppID..." -ForegroundColor Green
